...
2つの独立したエージェントを作成し、営業資料の管理と活動の可視化を実現します。
...
## 作業1：添付ファイル自動保存エージェント

重要顧客からの添付ファイルを自動でGoogle Driveに保存し、通知を送ります。
...
| 02 | Driveステップ（保存） | Drive - Upload file / 添付ファイルをDriveに保存 |
...
スプレッドシートのデータを読み込み、Geminiで分析して週次レポートを生成します。
...
添付ファイル保存時に自動でスプレッドシートにログを記録したい場合は、[応用編：スプレッドシート連携で営業ログを自動記録](advanced_スプレッドシート連携で営業ログを自動記録.md)を参照してください。
